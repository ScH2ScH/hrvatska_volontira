...
class UserRepository
{
    ...
    public function resetPassword($input)
    {
        ...
        if ($result) {
            ...
            Log::error('Error reseting password', (array)$user->errors());
        }
    ...
    {
        $user = User::where(array('confirmation_code' => $code))->first();

        if ($user) {
            $user->confirmed = true;
            $rules = User::$updateRulesWithoutPasswordConfirmation;
            $result = $user->updateUniques($rules);
            return $result;
        } else {
            return false;
        }
